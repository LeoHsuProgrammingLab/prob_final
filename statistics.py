...
def two_sample_t_test(data1: np.ndarray, data2: np.ndarray, siginificance_level: float, alternative: bool = False):
    # f-test
    f_stat, p_value, diffVar = F_test(data1, data2, 0.05, True)
    if (diffVar):
        # t-test
        print("Variances are different")
        t_stat, p_value = ttest_ind(data1, data2, equal_var=False, alternative='greater')
    else:
        t_stat, p_value = ttest_ind(data1, data2, equal_var=True, alternative='greater')
